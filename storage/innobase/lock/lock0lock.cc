	...
	que_thr_t*	thr)	/*!< in: query thread */
{
	...
	if (wait_for != NULL) {
		err = lock_table_enqueue_waiting(unsigned{mode} | flags, table,
						 thr
#ifdef WITH_WSREP
						 , wait_for
#endif
		...
		lock_table_create(table, unsigned{mode} | flags, trx);
	...
	que_thr_t*		thr)	/*!< in: query thread */
{
	...
	err = lock_rec_lock(FALSE, unsigned{mode} | gap_mode,
			    block, heap_no, index, thr);
